...
func (*DnsReporter) rp(analyzername string, filename string) error {
	analyzer := parse.Addons["analyzer"].(*dnsanalyzer.DnsAnalyzerAddon).DnsAnalyzers[analyzername]
	...
	fmt.Println()
	fmt.Printf("Percentage completed:  %f%%\n", 100.0*float64(*analyzer.Srsucccntp)/float64(*analyzer.Srcntp))
	fmt.Printf("Percentage lost:       %f%%\n", 100.0-100.0*float64(*analyzer.Srsucccntp)/float64(*analyzer.Srcntp))
	fmt.Println()
